...
#request large node 
#salloc -t 2-1 --mem-per-cpu 32G -n 21 --partition=largenode mpirun -n 1 Rscript BCA_SKAT_GERD+_MPI.R
...
mpi_compute_pvalue <- function(ngenes=40){
  ...
  for (ii in 1:nchunks)
  {
    ...
    idx=!is.na(pheno) & !is.na(sampletable$recurrent_HB_RF)& sampletable$recurrent_HB_RF==0
    pheno=pheno[idx]

...
#request large node 
#salloc -t 2-1 --mem-per-cpu 32G -n 21 --partition=largenode mpirun -n 1 Rscript BCA_GWAS_GERD_interaction_MPI.R
...
Covariate=sampletable[,colnames(sampletable) %in% c("sex","age","ev1_bca","ev2_bca","ev3_bca","ev4_bca","recurrent_HB_RF")]
...
compute_pvalue=function(i=1,pheno,genodat,covdat)
{
  ...
  covdat$recurrent_HB_RF <- 1-covdat$recurrent_HB_RF
  GE <- genotype*covdat$recurrent_HB_RF
  fit1 <- glm(I(pheno==2)~GE+genotype+recurrent_HB_RF+sex+age+ev1_bca+ev2_bca+ev3_bca+ev4_bca,family=binomial,data=covdat)
  fit2 <- glm(I(pheno==2)~recurrent_HB_RF+sex+age+ev1_bca+ev2_bca+ev3_bca+ev4_bca,family=binomial,data=covdat)
...
mpi_compute_pvalue <- function(nsnps=1000){
  ...
  for (ii in 1:nchunks)
  {
    ...
    idx=!is.na(pheno) & !is.na(Covariate1$recurrent_HB_RF)
    pheno=pheno[idx]
    genodat=genodat[,idx]
...
save(BCAout,file="/fh/fast/dai_j/BEACON/BEACON_GRANT/result/BCA_gwas_GERD_interaction_output.RData") 

...
validation_cases=sampletable$localid[sampletable$site %in% 30 & !is.na(sampletable$phenoEABE_bca)]
idx=match(validation_cases,sampletable$localid)
table(sampletable$phenoEABE_bca[idx])
validation_controls=sampletable$localid[which(sampletable$site %in% controlsites & sampletable$phenoEABE_bca==1)]
validationsamples=c(validation_cases,validation_controls)
discoverysamples=sampletable$localid[!is.na(sampletable$phenoEABE_bca)]
...
#write.table(discsamples,file="../result/discsamplesBEEA_plink.txt",row.names = F,col.names = F,sep="\t",quote=F)

BEEAsamples=sampletable1$localid[which(sampletable1$phenoEABE_bca==2)]
COsamples=sampletable1$localid[which(sampletable1$phenoEABE_bc==1)]
...
BEEAsamples=BEEAsamples[BEEAsamples %in% fam$V2]
filesforGWAS=function(casesamples=BEEAsamples,controlsamples=COsamples,prefix="Discovery_BEEA_CO")
{
...
filesforGWAS()
#check files:
tmp=read.table("../result/GWAS/Discovery_BEEA_CO_gwas.fam")
tmp1=read.table("../result/GWAS/Discovery_BEEA_CO_selectedsamples_pheno_plink.txt",header = T)

all(tmp$V2 %in% tmp1$IID) #T
udpate_fam=function(famfile="../result/GWAS/Discovery_BEEA_CO_gwas.fam",phenofile="../result/GWAS/Discovery_BEEA_CO_selectedsamples_pheno_plink.txt")
{
...
#after running gwas, update the result
update_gwas=function(gwasfile="/fh/fast/dai_j/BEACON/BEACON_GRANT/result/GWAS/Discovery_BEEA_CO_gwas.assoc.logistic",
                     bimfile="../result/GWAS/Discovery_BEEA_CO_gwas.bim")
{
  ...
  gwas$N=gwas$NMISS
  gwas1=as.data.frame(fread("/fh/fast/dai_j/BEACON/Meta_summary_stat/BEEA_BEACON_autosomes.txt"))
...
# system(cmd)
# 
BEEAmeta=as.data.frame(fread("../result/Discovery_Bonn_METAANALYSIS_BEEA_comsnp1.tbl"))
colnames(BEEAmeta)[which(colnames(BEEAmeta)=="P-value")]="P"
BEEABeacon=as.data.frame(fread("/fh/fast/dai_j/BEACON/BEACON_GRANT/result/BEEA_Discovery_BD_autosomes_comsnp_N.txt"))
idx=match(BEEAmeta$MarkerName,BEEABeacon$SNP)
BEEAmeta$chr=BEEABeacon$CHR[idx]
BEEAmeta$pos=BEEABeacon$position[idx]
write.table(BEEAmeta,file="/fh/fast/dai_j/BEACON/BEACON_GRANT/result/BEEA_Discovery_Bonn_imp_metastat.txt",col.names = T,
            row.names=F,quote=F,sep="\t")
cmd="gzip /fh/fast/dai_j/BEACON/BEACON_GRANT/result/BEEA_Discovery_Bonn_imp_metastat.txt"
system(cmd)
# 
...
write.table(valisamples,file="../result/validationBEEAsamples_plink.txt",row.names = F,col.names = F,sep="\t",quote=F)
keep_nohetsamples=function(prefix="/fh/fast/dai_j/BEACON/BEACON_GRANT/data/imputation/bca_1000g/validationBEEA_filter_noambiguous_QC")
{
...
generate_plink=function(prefix="/fh/fast/dai_j/BEACON/BEACON_GRANT/data/imputation/bca_1000g/validationBEEA_filter_noambiguous_QC",
                        opt="BEEA",prefix1="/fh/fast/dai_j/BEACON/BEACON_GRANT/result/Validation_BEEA",metadat=BEEAmeta)
{
  #
...
#generate_covariate()
generate_covariate(prefix="/fh/fast/dai_j/BEACON/BEACON_GRANT/result/Validation_BEEA")

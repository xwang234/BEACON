#change X to G for Rmpi
#multiply coeff to X

#outfolder="/fh/fast/dai_j/BEACON/BEACON_GRANT/result/dist500K_EAC2"
#outfolder="/fh/fast/dai_j/BEACON/BEACON_GRANT/result/dist500K_TCGA_PC4" #models stored for TCGA
#outfolder="/fh/fast/dai_j/BEACON/BEACON_GRANT/result/dist500K_GTEx_PC4" #models stored for GTEx
...
skat_p_noweight=function(Z,G,idx1,idx2)
{
  y=c(rep(1,length(idx1)),rep(0,length(idx2)))
  
  Z1=Z[c(idx1,idx2),,drop=F]
  X1=as.matrix(G[c(idx1,idx2),])
  tmp=rowSums(Z1)
  if (sum(tmp!=0) !=1)
  {
...
skat_p=function(Z,G,idx1,idx2)
{
  ...
  {
    obj.s<-SKAT_Null_Model(y ~ X1,out_type="D")
    out=SKATBinary(Z1, obj.s, weights.beta=c(1,25),kernel = "linear.weighted",r.corr=0) #1.207545e-05
    return(out$p.value)
  }else
  {
    return(NA)
...
skat_p_weight=function(Z,G,idx1,idx2,w)
{
  y=c(rep(1,length(idx1)),rep(0,length(idx2)))
  ...
  if (sum(tmp!=0) !=1)
  {
    ...
    #out<-SKAT(Z1, obj.s,weights.beta=c(1,1),r.corr=0) #1.207545e-05
    #out=SKATBinary(Z1, obj.s, weights.beta=c(1,25),kernel = "linear.weighted",r.corr=0) #0.04183234
    out=SKATBinary(Z1, obj.s, weights.beta=c(1,1),weights=w,kernel = "linear.weighted",r.corr=0) #1.207545e-05
    #out=SKATBinary(Z1, obj.s, method="SKATO", weights.beta=c(1,1),kernel = "linear.weighted") #4.244548e-05
    #out<-SKATBinary(Z1, obj.s, weights.beta=c(1,25),kernel = "IBS.weighted",r.corr=0) #0.06059779
    #out<-SKATBinary(Z1, obj.s, weights.beta=c(1,1),kernel = "2wayIX",r.corr=0) #5.994431e-05
    #out<-SKAT_CommonRare(Z1, obj.s) #0.000466221
...
G=sampletable[,colnames(sampletable) %in% c("sex","age","pc1","pc2","pc3","pc4")]
rownames(G)=sampletable$localid
G$age[is.na(G$age)]=mean(G$age,na.rm = T)

compute_p_arow=function(i,genemodel=res_min,bcagenotype,G)
{
  ...
  res$BE_p=skat_p(Z,G,idx1,idx2)
  idx1=which(sampletable$phenoEA_bc==2) #case
  idx2=which(sampletable$phenoEA_bc==1)
  res$EA_p=skat_p(Z,G,idx1,idx2)
  idx1=which(sampletable$phenoBE_bc==2)
  idx2=which(sampletable$phenoEA_bc==2)
  res$BEA_p=skat_p(Z,G,idx1,idx2)
  idx1=which(sampletable$phenoBE_bc==2 | sampletable$phenoEA_bc==2) #case
  idx2=which(sampletable$phenoBE_bc==1)
  res$BEEA_p=skat_p(Z,G,idx1,idx2)
  return(res)
}

compute_p_withcoeff_arow=function(i,genemodel=res_min,bcagenotype,G,opt="abs") #opt is not used
{
  genes=rownames(genemodel)[genemodel$glmflag==1]
  gene=genes[i]
  res=data.frame(BE_p=NA,EA_p=NA,BEA_p=NA,BEEA_p=NA,stringsAsFactors = F)
  ...
  idx=match(allsamples,rownames(Z))
  Z=Z[idx,,drop=F]
  w=1/sqrt(abs(selectedcoefs))
  #Z=t(t(Z)*(1/sqrt(abs(selectedcoefs)))) #add the coeff here
  
  
  #print(rankMatrix(Z)[[1]])
  idx1=which(sampletable$phenoBE_bc==2) #case
  idx2=which(sampletable$phenoBE_bc==1)
  res$BE_p=skat_p_weight(Z,G,idx1,idx2,w)
  idx1=which(sampletable$phenoEA_bc==2) #case
  idx2=which(sampletable$phenoEA_bc==1)
  res$EA_p=skat_p_weight(Z,G,idx1,idx2,w)
  idx1=which(sampletable$phenoBE_bc==2)
  idx2=which(sampletable$phenoEA_bc==2)
  res$BEA_p=skat_p_weight(Z,G,idx1,idx2,w)
  idx1=which(sampletable$phenoBE_bc==2 | sampletable$phenoEA_bc==2) #case
  idx2=which(sampletable$phenoBE_bc==1)
  res$BEEA_p=skat_p_weight(Z,G,idx1,idx2,w)
  return(res)
}


#run sequentially
compute_p_allrows=function(genemodel=res_min,bcagenotype,G)
{
  genemodel=genemodel[rownames(genemodel) %in% proteingenes,]
  genes=rownames(genemodel)[genemodel$glmflag==1]
  ...
  {
    if (i %%100 ==0) cat(i,"..")
    res[i,]=compute_p_arow(i,genemodel,bcagenotype,G)
  }
  rownames(res)=genes
  return(res)
}
skat_min=compute_p_allrows(genemodel=res_min,bcagenotype,G)
skat_1se=compute_p_allrows(genemodel = res_1se,bcagenotype,G)
save(skat_1se,skat_min,file=paste0(outfolder,"/skat_coef_dfweight_res.RData"))

compute_p_withcoeff_allrows=function(genemodel=res_min,bcagenotype,G,opt="abs")
{
  genes=rownames(genemodel)[genemodel$glmflag==1]
  res=data.frame(BE_p=rep(NA,length(genes)),EA_p=rep(NA,length(genes)),BEA_p=rep(NA,length(genes)),BEEA_p=rep(NA,length(genes)),stringsAsFactors = F)
  print(paste0("total genes: ",length(genes)))
  ...
  {
    if (i %%100 ==0) cat(i,"..")
    res[i,]=compute_p_withcoeff_arow(i,genemodel,bcagenotype,G,opt)
  }
  rownames(res)=genes
  return(res)
}
skat_min=compute_p_withcoeff_allrows(genemodel=res_min,bcagenotype,G,opt="abs")
skat_1se=compute_p_withcoeff_allrows(genemodel = res_1se,bcagenotype,G,opt="abs")
...
mpi_compute_p=function(genemodel=res_min)
{
  ...
  for (i in 1:nchunks)
  {
    ...
    tmp=mpi.parSapply(X=seq,FUN=compute_p_arow,genemodel=genemodel,bcagenotype=bcagenotype,G=G,job.num=njobs)
...
mpi_compute_p_coeff=function(genemodel=res_min)
{
  ...
  for (i in 1:nchunks)
  {
    ...
    tmp=mpi.parSapply(X=seq,FUN=compute_p_withcoeff_arow,genemodel=genemodel,bcagenotype=bcagenotype,G=G,job.num=njobs)
    if (length(tmp) %% 4 !=0) print(i)
    res1=matrix(unlist(tmp),ncol=4,byrow = T)
    #rownames(res1)=rownames(genemodel)[seq]
    res=rbind(res,res1)
...
save(skat_1se,skat_min,file=paste0(outfolder,"/skat_coef_abs_res.RData"))
#save(skat_1se,skat_min,file=paste0(outfolder,"/skat_coef_sqr_res.RData"))
...
compute_p_arow_adjgwas=function(i,genemodel=genemodel1,bcagenotype=bcagenotype1,G,selectedsnps_gwasdist=selectedsnps1_gwasdist)
{
  res=NA
  if (class(selectedsnps_gwasdist)=="data.frame")
  {
    ...
    idxgwas=which(colnames(bca_gwasdat) %in% selectedsnps_gwasdist$gwas)
    bca_gwasdat_=bca_gwasdat_[,idxgwas,drop=F]
    res=compute_p_arow(i,genemodel,bcagenotype,G=cbind(G,bca_gwasdat_))
  }
  return(res)
}
  ...
  for (i in 1:length(genes))
  {
    ...
    if (length(idx1)>0)
    {
      if (genemodel1$glmflag[idx1]==T)
      {
        ...
        skat_p1_adjgwas=compute_p_arow_adjgwas(i=which(rownames(genemodel1)[genemodel1$glmflag==1]==gene),genemodel = genemodel1,bcagenotype = bcagenotype1,G,selectedsnps_gwasdist=selectedsnps1_gwasdist)
    ...
    if (length(idx2)>0)
    {
      if (genemodel2$glmflag[idx2]==T)
      {
        ...
        {
          skat_p2_adjgwas=compute_p_arow_adjgwas(i=which(rownames(genemodel2)[genemodel2$glmflag==1]==gene),genemodel = genemodel2,bcagenotype = bcagenotype2,G,selectedsnps_gwasdist=selectedsnps2_gwasdist)

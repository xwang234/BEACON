...
Covariate=sampletable[,colnames(sampletable) %in% c("sex","age","ev1_bca","ev2_bca","ev3_bca","ev4_bca","cig_smk_ever")]
...
compute_pvalue=function(i=1,pheno,genodat,covdat)
{
  ...
  GE <- genotype*covdat$cig_smk_ever
  fit1 <- glm(I(pheno==2)~GE+genotype+cig_smk_ever+sex+age+ev1_bca+ev2_bca+ev3_bca+ev4_bca,family=binomial,data=covdat)
  fit2 <- glm(I(pheno==2)~cig_smk_ever+sex+age+ev1_bca+ev2_bca+ev3_bca+ev4_bca,family=binomial,data=covdat)
...
mpi_compute_pvalue <- function(nsnps=1000){
  ...
  for (ii in 1:nchunks)
  {
    ...
    idx=!is.na(pheno) & !is.na(Covariate1$cig_smk_ever)
    pheno=pheno[idx]
    genodat=genodat[,idx]
...
save(BCAout,file="/fh/fast/dai_j/BEACON/BEACON_GRANT/result/BCA_gwas_cig_interaction_output.RData") 

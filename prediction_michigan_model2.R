...
# #get all the data
# # #load copynumber
#include disease,gender to covariate
...
selectlambda=function(x,y,numlambda=100,ntime=10,opt="numeric",nfolds=10)
{
  library(glmnet)
  ...
  for (i in 1:ntime){
    ...
    {
      set.seed(i)
      cvfit <- cv.glmnet(data.matrix(x),y,family="binomial",nlambda=numlambda,nfolds=nfolds)
    }
    if (opt=="numeric")
    {
      set.seed(i)
      cvfit <- cv.glmnet(data.matrix(x),y,nlambda=numlambda,nfolds=nfolds)
    }
...
compute_cor_arow=function(i,opt="1se",ncv=10,distcutoff=1e6)
{
  ...
  if (length(idx)>1)
  {
    ...
    set.seed(i+10000)
    #cvfit <- cv.glmnet(data.matrix(Xall),Y,nlambda=100,nfolds=10)
    fit=glmnet(data.matrix(Xall),Y,nlambda = 100)
    lambda_1varsel=fit$lambda[which(fit$df>0)[1]] #lambda value when 1 snp variable is selected should be greater than this
    
    lambdas=selectlambda(data.matrix(Xall),Y,nfolds=10)
    
    if (lambdas$lambda.1se<lambda_1varsel) #if glmnet 1se selected some variables
    {
      glmcoeff=as.matrix(coef(fit,s=lambdas$lambda.1se))
      if (sum(rownames(glmcoeff)[glmcoeff[,1]!=0] %in% colnames(X))>0) #if SNPs were selected
      {
        if (opt=="min")
        {
          lamba_sel=lambdas$lambda.min
        }else
        {
          lamba_sel=lambdas$lambda.1se
        }
        glmcoeff=as.matrix(coef(fit,s=lamba_sel))
        glmleftsnp=rownames(glmcoeff)[rownames(glmcoeff) %in% colnames(X) & glmcoeff[,1]!=0] #snps left in glm model
        idx1=match(glmleftsnp,rownames(glmcoeff))
...
#compute
test=compute_cor_arow(i=6468,opt="1se",ncv=10,distcutoff = 1e6)
...
mpi.bcast.Robj2slave(selectlambda)

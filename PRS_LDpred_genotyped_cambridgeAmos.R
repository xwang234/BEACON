...
EAmeta=as.data.frame(fread("../result/Beacon_Bonn_METAANALYSIS_EA_comsnp1.tbl"))
colnames(EAmeta)[which(colnames(EAmeta)=="P-value")]="P"
EABeacon=as.data.frame(fread("/fh/fast/dai_j/BEACON/BEACON_GRANT/result/EA_Beacon_autosomes_comsnp_N.txt"))
idx=match(EAmeta$MarkerName,EABeacon$SNP)
EAmeta$chr=EABeacon$CHR[idx]
EAmeta$pos=EABeacon$position[idx]
write.table(EAmeta,file="/fh/fast/dai_j/BEACON/BEACON_GRANT/result/EA_Beacon_Bonn_metastat.txt",col.names = T,
            row.names=F,quote=F,sep="\t")
...
EAmeta=as.data.frame(fread("../result/Beacon_Bonn_METAANALYSIS_EA_info_comsnp1.tbl"))
colnames(EAmeta)[which(colnames(EAmeta)=="P-value")]="P"
EABeacon=as.data.frame(fread("/fh/fast/dai_j/BEACON/BEACON_GRANT/result/EA_BEACON_autosomes_N.txt"))
idx=match(EAmeta$MarkerName,EABeacon$SNP)
EAmeta$chr=EABeacon$CHR[idx]
EAmeta$pos=EABeacon$position[idx]
write.table(EAmeta,file="/fh/fast/dai_j/BEACON/BEACON_GRANT/result/EA_Beacon_Bonn_info_metastat.txt",col.names = T,
            row.names=F,quote=F,sep="\t")
cmd="gzip /fh/fast/dai_j/BEACON/BEACON_GRANT/result/EA_Beacon_Bonn_info_metastat.txt"
system(cmd)
#add case_N, control_N
tmp=fread(input="/fh/fast/dai_j/BEACON/BEACON_GRANT/result/EA_Beacon_Bonn_info_metastat.txt.gz")
tmp$case_N=3121
tmp$control_N=5722
write.table(tmp,file="/fh/fast/dai_j/BEACON/BEACON_GRANT/result/EA_Beacon_Bonn_info_metastat.txt",col.names = T,
            row.names=F,quote=F,sep="\t")
cmd="gzip /fh/fast/dai_j/BEACON/BEACON_GRANT/result/EA_Beacon_Bonn_info_metastat.txt"
system(cmd)
...
generate_plink(opt="EA",prefix1="/fh/fast/dai_j/BEACON/BEACON_GRANT/result/CambAmos_EA_genotyped",metadat=EAmeta)
...
#generate_covariate()
generate_covariate(prefix="/fh/fast/dai_j/BEACON/BEACON_GRANT/result/CambAmos_EA_genotyped")
...
plot_ROC(prefix="/fh/fast/dai_j/BEACON/BEACON_GRANT/result/CambAmos_EA_genotyped",opt1="Cambridge")
# [1] "p=1:auc_prs=0.785"
# [1] "p=0.3:auc_prs=0.786"
# [1] "p=0.1:auc_prs=0.786"
# [1] "p=0.03:auc_prs=0.785"
# [1] "p=0.01:auc_prs=0.772"
...
plot_ROC(prefix="/fh/fast/dai_j/BEACON/BEACON_GRANT/result/CambAmos_EA",opt1="Cambridge")
# [1] "p=1:auc_prs=0.783"
# [1] "p=0.3:auc_prs=0.783"
# [1] "p=0.1:auc_prs=0.784"
# [1] "p=0.03:auc_prs=0.784"
# [1] "p=0.01:auc_prs=0.781"
...
PRS_odds(prefix="/fh/fast/dai_j/BEACON/BEACON_GRANT/result/CambAmos_EA_genotyped",p=0.1)
# [1] "2th quantile:"
# Waiting for profiling to be done...
# 2.5 %   97.5 % 
...
plotroc=function(opt1="CambAmos",EAfile="../result/CambAmos_EA_genotyped.PRS.txt")
{
  library(readxl)
  sampletable=read_excel("/fh/fast/dai_j/BEACON/BEACON_GRANT/data/PLINKinputCombo_bca_07Feb2018.xls",1)
  sampletable <- data.frame(sampletable)
  ...
  }else
  {
    ...
    #fit2 <- glm(I(phenoEA_bca==2)~age+sex+recurrent_HB_RF+bmi_recent_healthy+cig_smk_ever+nsaid_ever,family=binomial,data=sampletable[sampletable$site>=30,],y=T)
    #summary(fit2)
    dat=sampletable[,c("phenoEA_bca","age","sex","recurrent_HB_RF","cig_smk_ever","nsaid_ever","EA.prs","site")]
    idx=complete.cases(dat)
    dat=dat[idx,]
    fit3 <- glm(I(phenoEA_bca==2)~age+sex+recurrent_HB_RF+cig_smk_ever+nsaid_ever+EA.prs,family=binomial,data=dat[dat$site>=30,],y=T)
    #summary(fit3)
  }

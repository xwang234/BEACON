...
##### Aug 2021                                                          #######
###############################################################################
#update validation process, by adding more missing SNPs
...
TWAS_SKAT_gene2 <- function(genename,res_min,cvmin=NULL,phenotype=NULL,phenotypepos=NULL,snp=NULL,snppos=NULL,bcagenotype=NULL,covariate=NULL,verbose=0) {
  ...
  refbim=fread(paste0(thousanddir,"ALL.chr",chr,".phase3_shapeit2_mvncall_integrated_v5a.20130502.genotypes.EUR.bim"))
  #refsnps=intersect(rsid[!is.na(rsid)],refbim$V2)
  refraw=fread(input=paste0(thousanddir,"ALL.chr",chr,".phase3_shapeit2_mvncall_integrated_v5a.20130502.genotypes.EUR.traw.gz"))
  ...
  missingsnps=selsnpstr0[which(!selsnpstr0 %in% EA_Bonnsummarydatstr)]
  if (length(missingsnps)>0)
  {
    ...
    for (k in 1:nrow(refreplacesnps1))
    {
      ...
      # idx=which(EA_Bonnsummarydat$chr==chr)
      # idx1=which.min(abs(EA_Bonnsummarydat$position[idx]-tmp1))
      # refreplacesnps1$replacestr0[k]=EA_Bonnsummarydatstr[idx[idx1]]
      # refreplacesnps1$dist[k]=min(abs(EA_Bonnsummarydat$position[idx]-tmp1))
      idx1=which(bcabim1str0==refreplacesnps1$snpstr0[k])
      idx_keep=which(abs(bcabim1$V4-tmp1)<500000)
      idx2=which(bcabim1str0[idx_keep] %in% EA_Bonnsummarydatstr)
      tmp2=t(bcagenotype1[idx_keep[idx2],])
      tmp3=cor(tmp2,unlist(bcagenotype1[idx1,]))
  ...
  tmp0=intersect(selsnpstr0,EA_Bonnsummarydatstr)
  tmp=c(tmp0,refreplacesnps1$replacestr0[refreplacesnps1$cor>0.6])
  ...
  missingsnps=selsnpstr0[which(!selsnpstr0 %in% BEEA_Bonnsummarydatstr)]
  if (length(missingsnps)>0)
  {
    ...
    for (k in 1:nrow(refreplacesnps1))
    {
      ...
      idx1=which.min(abs(BEEA_Bonnsummarydat$position[idx]-tmp1))
      refreplacesnps1$replacestr0[k]=BEEA_Bonnsummarydatstr[idx[idx1]]
      refreplacesnps1$dist[k]=min(abs(BEEA_Bonnsummarydat$position[idx]-tmp1))
      
    }
    
    tmp0=intersect(selsnpstr0,BEEA_Bonnsummarydatstr)
    tmp=c(tmp0,refreplacesnps1$replacestr0[refreplacesnps1$dist<=50])
  }
  ...
  #if there are missing snps, use the closest snps in BonnBE
  ...
  if (length(missingsnps)>0)
  {
    ...
    tmp0=intersect(selsnpstr0,BE_Bonnsummarydatstr)
    tmp=c(tmp0,refreplacesnps1$replacestr0[refreplacesnps1$dist<=50])
  }
...
genelist$validated[idx]=T
idx=order(genelist$validated,decreasing = T)
write.csv(genelist[idx,],file="../result/NEWGenesValidate_allcovar_ALLFDR_1213.csv",quote=F,row.names = F)
#update AllFDR

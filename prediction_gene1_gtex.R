#!/usr/bin/env Rscript
# #USE 225 GTEx junction samples (WGS genotypes), work on one gene
...
compute_cor_arow=function(gene="DDX49",opt="1se",ncv=10,distcutoff=1e6)
{
  i=which(rownames(phenotype)==gene)
  ...
  if (length(idx)>1)
  {
    ...
    cvfit=tryCatch(
      {
        cv.glmnet(data.matrix(Xall),Y,nlambda=100,nfolds=10, penalty.factor=penalty,alpha=1)
      },
      error=function(e)
      {
        return(F)
      ...
      if (cvfit$lambda.min<=lambda_1varsel) #if glmnet min selected some variables
      {
        ...
        if (sum(rownames(glmcoeff)[glmcoeff[,1]!=0] %in% colnames(X))>0) #if SNPs were selected
        {
          ...
          if (numvar>0)
          {
            ...
            if (sum(rownames(lmcoeff)=="race")>0) p_race=lmcoeff[which(rownames(lmcoeff)=="race"),4]
  ...
  return(list(r2=r2,glmflag=glmflag,numvar=numvar,numsnp=length(idx),selectedsnps=selectedsnps,selectedsnps_coeff=selectedsnps_coeff,
              p_race=p_race,p_age=p_age,p_gender=p_gender))
}
...
# [1] 1850
# $selectedsnps
# [1] "19:19348311_T|19:18906534_C"
# $selectedsnps_coeff
# [1] "0.233286517229078|-0.148622689609117"
# $p_race
# [1] 0.5010563
# $p_age
# [1] 0.6822489
...
# DDX49 0.4136444 0.5716331 0.9168984 0.3159172
...
impsnps=unlist(strsplit("19:18774810_G|19:18782197_C","|",fixed = T)) #from imputed data
#SNP6 result
...
# p_gender
# DDX49 0.6526358
twosnps=unlist(strsplit("19:18782377_A|19:18749354_A","|",fixed = T)) #from snp6 data
#check correlation------------
...
colnames(TCGAknownsnps1)=c("19:18804294_TG","19:18803172_T","19:18817903_A")
...
snp1=rbind(snp,t(TCGAknownsnps1[,idx]))
snppos1=rbind(snppos,data.frame(chr=rep(19,2),pos=c(18804294,18817903),mag=NA,min=NA,stringsAsFactors = F))
rownames(snppos1)[(nrow(snppos1)-1):nrow(snppos1)]=c("19:18804294","19:18817903")
snp=snp1
snppos=snppos1

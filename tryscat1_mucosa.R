...
#run sequentially
compute_p_allrows=function(genemodel=res_min,bcagenotype,X)
{
  genes=rownames(genemodel)[genemodel$glmflag==1]
  ...
  {
    if (i %%100 ==0) cat(i,"..")
    res[i,]=compute_p_arow(i,genemodel,bcagenotype,X)
  }
  rownames(res)=genes
  return(res)
}
skat_min=compute_p_allrows(genemodel=res_min,bcagenotype,X)
skat_1se=compute_p_allrows(genemodel = res_1se,bcagenotype,X)

#salloc -t 1-1 --mem-per-cpu 48G -n 11 --partition=largenode mpirun -n 1 /app/easybuild/software/R/3.3.3-foss-2016b-fh1/bin/R --interactive
...
mpi_compute_p=function(genemodel=res_min)
{
  ...
  for (i in 1:nchunks)
  {
    ...
    tmp=mpi.parSapply(X=seq,FUN=compute_p_arow,genemodel=genemodel,bcagenotype=bcagenotype,X=X,job.num=njobs)
...
fdr_fwer_res=compute_fwer_fdr(dat=skat_min2[rownames(skat_min2) %in% proteingenes,])

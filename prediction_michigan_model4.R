...
compute_cor_arow=function(i,opt="1se",ncv=10,distcutoff=1e6)
{
  ...
  if (length(idx)>1)
  {
    ...
    {
      set.seed(ii+100)
      cvfit <- cv.glmnet(data.matrix(Xall),Y,nlambda=100,nfolds=10,alpha=1)
      print(cvfit$lambda.1se)
    }
    cvfit <- cv.glmnet(data.matrix(Xall),Y,nlambda=100,nfolds=10,alpha=1)
    fit=glmnet(as.matrix(Xall),Y,nlambda = 100,alpha=1)
    lambda_1varsel=fit$lambda[which(fit$df>0)[1]] #lambda value when 1 snp variable is selected should be greater than this
    ...
    if (cvfit$lambda.1se<lambda_1varsel) #if glmnet 1se selected some variables
    {
      ...
      if (sum(rownames(glmcoeff)[glmcoeff[,1]!=0] %in% colnames(X))>0) #if SNPs were selected
      {
        ...
        test=X[,idx1]
        # test1=colnames(test)
        # for (ii in 1:length(test1))
        # {
        #   tmp=unlist(strsplit(test1[ii],"_"))
        #   tmp1=unlist(strsplit(tmp[1],":"))
        #   test1[ii]=paste0(tmp[1],"-",tmp1[2])
        # }
        # test1=paste0(test1,collapse = "\n")
        # filename=paste0(outfolder,"/prediction_snps_tabix_test.txt")
        # fileCon=file(filename)
        # writeLines(test1,fileCon)
        # close(fileCon)
        test1=cor(X[,idx1])
        numvar=length(glmleftsnp)
        if (numvar>0)
...
#compute
test=compute_cor_arow(i=14680,opt="1se",ncv=10,distcutoff = 1e6)
...
#test identical genotypes (found in TCGA) in BCA
tmp=read.table("/fh/fast/dai_j/BEACON/BEACON_GRANT/result/dist500K_EAC/chr19_test.raw",stringsAsFactors = F,header = T)
tmp=tmp[,7:ncol(tmp)]
colnames(tmp)=gsub(".",":",colnames(tmp),fixed=T)
colnames(tmp)=gsub("X","",colnames(tmp))
idx=match(colnames(test1),colnames(tmp))
tmp=tmp[,idx]
tmp1=cor(tmp)

#!/usr/bin/env Rscript
# generate data for LDpred,for genotyped cambamos data 
#use Cambridge+AMOS as the validation, only work on EA
#BEACON GWAS exclude AMOS samples
...
fam=read.table("/fh/fast/dai_j/BEACON/BEACON_GRANT/result/beacon_filtered_30Nov2018_flip_noambiguous.fam",header = F,stringsAsFactors = F)
EAsamples=sampletable1$localid[which(sampletable1$localid %in% fam$V2 & sampletable1$phenoEA_bc==2)]
COsamples=sampletable1$localid[which(sampletable1$localid %in% fam$V2 &  sampletable1$phenoEA_bc==1)]
filesforGWAS=function(casesamples=EAsamples,controlsamples=COsamples,prefix="Beacon_genotyped_EA_CO")
{
...
filesforGWAS()
#check files:
tmp=read.table("../result/GWAS/Beacon_genotyped_gwas.fam")
tmp1=read.table("../result/GWAS/Beacon_genotyped_EA_CO_selectedsamples_pheno_plink.txt",header = T)
all(tmp$V2 %in% tmp1$IID) #T
udpate_fam=function(famfile="../result/GWAS/Beacon_genotyped_EA_CO_gwas.fam",phenofile="../result/GWAS/Beacon_genotyped_EA_CO_selectedsamples_pheno_plink.txt")
{
...
update_gwas=function(gwasfile="/fh/fast/dai_j/BEACON/BEACON_GRANT/result/GWAS/Beacon_genotyped_EA_CO_gwas.assoc.logistic",
                     bimfile="../result/GWAS/Beacon_genotyped_EA_CO_gwas.bim")
{
...
EAmeta=as.data.frame(fread("../result/Beacon_Bonn_METAANALYSIS_EA_genotypedNoAmos_comsnp1.tbl"))
colnames(EAmeta)[which(colnames(EAmeta)=="P-value")]="P"
EABeacon=as.data.frame(fread("/fh/fast/dai_j/BEACON/BEACON_GRANT/result/EA_Beacon_genotyped_BB_autosomes_comsnp_N.txt"))
idx=match(EAmeta$MarkerName,EABeacon$SNP)
EAmeta$chr=EABeacon$CHR[idx]
EAmeta$pos=EABeacon$position[idx]
write.table(EAmeta,file="/fh/fast/dai_j/BEACON/BEACON_GRANT/result/EA_Beacon_Bonn_genotyped_noAmos_metastat.txt",col.names = T,
            row.names=F,quote=F,sep="\t")
cmd="gzip /fh/fast/dai_j/BEACON/BEACON_GRANT/result/EA_Beacon_Bonn_genotyped_noAmos_metastat.txt"
system(cmd)
# #add case_N, control_N
# tmp=fread(input="/fh/fast/dai_j/BEACON/BEACON_GRANT/result/EA_Beacon_Bonn_metastat.txt.gz")
# tmp$case_N=3121
# tmp$control_N=5722
# write.table(tmp,file="/fh/fast/dai_j/BEACON/BEACON_GRANT/result/EA_Beacon_Bonn_metastat.txt",col.names = T,
#             row.names=F,quote=F,sep="\t")
# cmd="gzip /fh/fast/dai_j/BEACON/BEACON_GRANT/result/EA_Beacon_Bonn_metastat.txt"
# system(cmd)
...
#Pick Cambridge and Amos from BCA
library(readxl)
#step 2---
sampletable=read_excel("/fh/fast/dai_j/BEACON/BEACON_GRANT/data/PLINKinputCombo_bca_07Feb2018.xls",1)
sampletable=as.data.frame(sampletable)
#site 30:Cambridge, 55:AMOS
idx=which(sampletable$site %in% c(30,55))
cambamossamples=sampletable[idx,1:2]
table(sampletable$phenoBE_bca[idx])
# -9    1    2 
# 1057 1022  882 
table(sampletable$phenoEA_bca[idx])
...
# -9    1    2 
# 54 1022 1885  
write.table(cambamossamples,file="../result/cambamossamples_plink.txt",row.names = F,col.names = F,sep="\t",quote=F)


keep_nohetsamples=function(prefix="/fh/fast/dai_j/BEACON/BEACON_GRANT/result/cambamos_filtered_30Nov2018_flip_noambiguous_QC")
{
...
#step 4----
#generate cambamos BE/EA/BEEA plink files
generate_plink=function(prefix="/fh/fast/dai_j/BEACON/BEACON_GRANT/result/cambamos_filtered_30Nov2018_flip_noambiguous_QC",
...
generate_plink(opt="EA",prefix1="/fh/fast/dai_j/BEACON/BEACON_GRANT/result/CambAmos_EA_genotyped_noAmos",metadat=EAmeta)
...
#generate_covariate()
generate_covariate(prefix="/fh/fast/dai_j/BEACON/BEACON_GRANT/result/CambAmos_EA_genotyped_noAmos")
...
# Waiting for profiling to be done...
# 2.5 %   97.5 % 
#   3.841514 2.942396 5.038873 
# [1] "3th quantile:"
# Waiting for profiling to be done...
# 2.5 %    97.5 % 
#   8.616829  6.533381 11.438501 
# [1] "4th quantile:"
# Waiting for profiling to be done...
# 2.5 %   97.5 % 
#   18.65469 13.68258 25.70458 
...
plotroc=function(opt1="CambAmos",EAfile="../result/CambAmos_EA_genotyped_noAmos.PRS.txt")
{
  library(readxl)
  sampletable=read_excel("/fh/fast/dai_j/BEACON/BEACON_GRANT/data/PLINKinputCombo_bca_07Feb2018.xls",1)
  sampletable <- data.frame(sampletable)
  ...
  }else
  {
    ...
    #fit2 <- glm(I(phenoEA_bca==2)~age+sex+recurrent_HB_RF+bmi_recent_healthy+cig_smk_ever+nsaid_ever,family=binomial,data=sampletable[sampletable$site>=30,],y=T)
    #summary(fit2)
    dat=sampletable[,c("phenoEA_bca","age","sex","recurrent_HB_RF","bmi_recent_healthy","cig_smk_ever","nsaid_ever","EA.prs","site")]
    idx=complete.cases(dat)
    dat=dat[idx,]
    fit3 <- glm(I(phenoEA_bca==2)~age+sex+recurrent_HB_RF+bmi_recent_healthy+cig_smk_ever+nsaid_ever+EA.prs,family=binomial,data=dat[dat$site>=30,],y=T)
    #summary(fit3)
  }

...
extract_vcf(){
  ...
  xargs -a $tabixreg -I {} tabix -f /fh/fast/dai_j/BEACON/BEACON_GRANT/data/imputation/bca_1000g/chr${chr}.dose.vcf.gz {} >> $outfolder/chr${chr}.select.dose.vcf
}

for chr in {1..22}; 
do 
  if [[ -f "$outfolder/prediction_snps_tabix_chr${chr}.txt" ]];
  then
    extract_vcf $chr & 
  fi
done

#!/usr/bin/env Rscript
# generate data for LDpred,for imputed cambamos data 
...
fam=read.table("/fh/fast/dai_j/BEACON/BEACON_GRANT/data/imputation/bca_1000g/beacon_filter_noambiguous.fam",header = F,stringsAsFactors = F)
...
filesforGWAS=function(casesamples=EAsamples,controlsamples=COsamples,prefix="Beacon_EA_CO")
{
...
filesforGWAS()
#check files:
tmp=read.table("../result/GWAS/Beacon_EA_CO_gwas.fam")
tmp1=read.table("../result/GWAS/Beacon_EA_CO_selectedsamples_pheno_plink.txt",header = T)
all(tmp$V2 %in% tmp1$IID) #T
udpate_fam=function(famfile="../result/GWAS/Beacon_EA_CO_gwas.fam",phenofile="../result/GWAS/Beacon_EA_CO_selectedsamples_pheno_plink.txt")
{
...
update_gwas=function(gwasfile="/fh/fast/dai_j/BEACON/BEACON_GRANT/result/GWAS/Beacon_EA_CO_gwas.assoc.logistic",
                     bimfile="../result/GWAS/Beacon_EA_CO_gwas.bim")
{
...
# system(cmd)
# 
EAmeta=as.data.frame(fread("../result/Beacon_Bonn_METAANALYSIS_EA_impNoAmos_comsnp1.tbl"))
colnames(EAmeta)[which(colnames(EAmeta)=="P-value")]="P"
EABeacon=as.data.frame(fread("/fh/fast/dai_j/BEACON/BEACON_GRANT/result/EA_Beacon_imp_BB_autosomes_comsnp_N.txt"))
idx=match(EAmeta$MarkerName,EABeacon$SNP)
EAmeta$chr=EABeacon$CHR[idx]
EAmeta$pos=EABeacon$position[idx]
write.table(EAmeta,file="/fh/fast/dai_j/BEACON/BEACON_GRANT/result/EA_Beacon_Bonn_imp_noAmos_metastat.txt",col.names = T,
            row.names=F,quote=F,sep="\t")
cmd="gzip /fh/fast/dai_j/BEACON/BEACON_GRANT/result/EA_Beacon_Bonn_imp_noAmos_metastat.txt"
system(cmd)
# 
...
#step2-------
keep_nohetsamples=function(prefix="/fh/fast/dai_j/BEACON/BEACON_GRANT/data/imputation/bca_1000g/cambamos_filter_noambiguous_QC")
{
...
generate_plink=function(prefix="/fh/fast/dai_j/BEACON/BEACON_GRANT/data/imputation/bca_1000g/cambamos_filter_noambiguous_QC",
...
generate_plink(opt="EA",prefix1="/fh/fast/dai_j/BEACON/BEACON_GRANT/result/CambAmos_EA_noAmos",metadat=EAmeta)
...
#generate_covariate()
generate_covariate(prefix="/fh/fast/dai_j/BEACON/BEACON_GRANT/result/CambAmos_EA_noAmos")

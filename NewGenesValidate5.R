...
#allelle allignment summarydat and refgenotype, this was used in the manuscript
TWAS_SKAT_gene2 <- function(genename,res_min,cvmin=NULL,phenotype=NULL,phenotypepos=NULL,snp=NULL,snppos=NULL,bcagenotype=NULL,covariate=NULL,verbose=0) {
  ...
  if (length(missingsnps)>0)
  {
    ...
    for (k in 1:nrow(refreplacesnps1))
    {
      ...
      if (length(idx2)>0)
      {
        ...
        idx3=which.min(abs(EA_Bonnsummarydat$position[idx]-tmp1))
  ...
  if (length(missingsnps)>0)
  {
    ...
    for (k in 1:nrow(refreplacesnps1))
    {
      ...
      idx1=which(bcabim1str0==refreplacesnps1$snpstr0[k])
      idx_keep=which(abs(bcabim1$V4-tmp1)<=5000)
      idx2=which(bcabim1str0[idx_keep] %in% BEEA_Bonnsummarydatstr)
      if (length(idx2)>0)
      {
        tmp2=t(bcagenotype1[idx_keep[idx2],7:ncol(bcagenotype1)])
        tmp3=cor(tmp2,unlist(bcagenotype1[idx1,7:ncol(bcagenotype1)]))
        names(tmp3)=bcabim1str0[idx_keep[idx2]]
        idx2=which.max(tmp3)
        idx=which(BEEA_Bonnsummarydat$chr==chr)
        idx3=which.min(abs(BEEA_Bonnsummarydat$position[idx]-tmp1))
        refreplacesnps1$replacestr0[k]=names(tmp3)[idx2]
        tmp4=as.integer(unlist(strsplit(names(tmp3)[idx2],":"))[2])
        refreplacesnps1$dist[k]=abs(tmp4-tmp1)
        refreplacesnps1$cor[k]=tmp3[idx2]
  ...
  if (length(missingsnps)>0)
  {
    ...
    for (k in 1:nrow(refreplacesnps1))
    {
      ...
      idx1=which(bcabim1str0==refreplacesnps1$snpstr0[k])
      idx_keep=which(abs(bcabim1$V4-tmp1)<=5000)
      idx2=which(bcabim1str0[idx_keep] %in% BE_Bonnsummarydatstr)
      if (length(idx2)>0)
      {
        tmp2=t(bcagenotype1[idx_keep[idx2],7:ncol(bcagenotype1)])
        tmp3=cor(tmp2,unlist(bcagenotype1[idx1,7:ncol(bcagenotype1)]))
        ...
        idx3=which.min(abs(BE_Bonnsummarydat$position[idx]-tmp1))
        refreplacesnps1$replacestr0[k]=names(tmp3)[idx2]
        tmp4=as.integer(unlist(strsplit(names(tmp3)[idx2],":"))[2])
        refreplacesnps1$dist[k]=abs(tmp4-tmp1)
        refreplacesnps1$cor[k]=tmp3[idx2]
...
genelist$validated[idx]=T
idx=order(genelist$validated,decreasing = T)
write.csv(genelist[idx,],file="../result/NEWGenesValidate_allcovar_ALLFDR_01_06.csv",quote=F,row.names = F)
#update AllFDR

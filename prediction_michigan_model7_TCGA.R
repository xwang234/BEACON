...
# compute_cor_arow_old=function(i,opt="1se",ncv=10,distcutoff=1e6)
# {
# 
#   Y=unlist(phenotype[i,]) #geneexp
#   r2=NA
...
#compute
test=compute_cor_arow(i=4777,ncv=10,distcutoff = 5e5)
compute_cor_arow(i=1186,ncv=10,distcutoff = 5e5)
test1=compute_cor_arow_old(i=4777,opt="min",ncv=10,distcutoff = 5e5)

# #parallel
#salloc -t 6-1  -n 41 mpirun -n 1 R --interactive
#salloc -t 4-1  -n 61 mpirun -n 1 /app/easybuild/software/R/3.3.3-foss-2016b-fh1/bin/R --interactive
...
mpi_compute_cor=function(ncv=10,distcutoff=5e5)
{
  ...
  for (chr in 1:22)
  {
    ...
    copynumber=allcopynumber[idx,]
    mutation=allmutation[idx,]
...
# mpi_compute_cor_old=function(rows=1:nrow(phenotypepos),opt="min",ncv=10,distcutoff=1e6)
# {
#   res=NULL
#   n=1000
#   nchunks=ceiling(length(rows)/n)
...
mpi_predict_geneexp=function(modeltable=res_min)
{
  ...
  nchunks=ceiling(sum(modeltable$numselectedsnp)/1000) #each chunk picks ~1000 snps

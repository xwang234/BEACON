...
compute_cor_arow=function(i,opt="1se",ncv=5)
{
  ...
  idx=which(tmp<1e6) #select snp within 1M of geneexp
...
compute_cor_arow=function(i,opt="min",ncv=10,distcutoff=5e5)
{
...
save(res,res_1se,file="../result/prediction_model_Jan7.RData")
...
#salloc -t 1-1 --mem-per-cpu 16G -n 51  mpirun -n 1 /app/easybuild/software/R/3.3.3-foss-2016b-fh1/bin/R --interactive
load("/fh/fast/dai_j/BEACON/BEACON_GRANT/result/TCGAdatafor_prediction_michigan.RData")
library(Rmpi)
...
mpi_compute_cor=function(rows,opt="min",ncv=10)
{
  res=NULL
  n=1000
...
mpi_compute_cor=function(rows=1:nrow(phenotypepos),opt="min",ncv=10,distcutoff=5e5)
{
  res=NULL
  n=1000
  nchunks=ceiling(length(rows)/n)
...
#save(res_min,res_1se,res_min_10cv,res_1se_10cv,file="../result/preidiction_michigan_model.RData")
...
extract_snp=function(dat=rbind(res_min,res_1se))
{
  ...
  for (i in 1:length(unique(res$chr)))
  {
    ...
    filename=paste0("/fh/fast/dai_j/BEACON/BEACON_GRANT/result/prediction_snps_tabix_chr",chr,".txt")
...
{
  cat(i,'..')
  tmp=fread(paste0("/fh/fast/dai_j/BEACON/BEACON_GRANT/data/imputation/bca_1000g/chr",i,"_select.raw"),header=T)
  tmp=as.data.frame(tmp)
  rownames(tmp)=tmp$IID
  tmp=tmp[,7:ncol(tmp)]
  tmp1=fread(paste0("/fh/fast/dai_j/BEACON/BEACON_GRANT/data/imputation/bca_1000g/chr",i,"_select.bim"))
  if (i==1)
  {
    bcagenotype=tmp
    bcabim=tmp1
...
save(bcagenotype,bcabim,file="../result/bca_extractgenotype.RData")
...
n=sum(res_min$glmflag==1)
predicted_geneexp_min=NULL
for (i in 1:n)
{
  if (i %%100==0) cat (i,'..')
  tmp=predict_geneexp(i)
  predicted_geneexp_min=rbind(predicted_geneexp_min,tmp)
}

##salloc -t 1-1 --mem-per-cpu 32G -n 16 --partition=largenode mpirun -n 1 /app/easybuild/software/R/3.3.3-foss-2016b-fh1/bin/R --interactive
load("/fh/fast/dai_j/BEACON/BEACON_GRANT/result/TCGAdatafor_prediction_michigan.RData") #phenotypepos
load("../result/preidiction_michigan_model.RData") #models
load("../result/bca_extractgenotype.RData") #extracted genotype and bcabim
library(Rmpi)
njobs=mpi.universe.size() - 1
print(njobs)
mpi.spawn.Rslaves(nslaves=njobs,needlog = F)
...
#mpi.bcast.Robj2slave(res_min_10cv)
#mpi.bcast.Robj2slave(res_1se_10cv)
#mpi.remote.exec(load("../result/bca_extractgenotype.RData")) #too big
...
save(predict_min,predict_1se,file="../result/bca_predict_geneexp.RData")
#after run mpi_predict_geneexp
load("../result/bca_predict_geneexp.RData")
...
pvalue_arow=function(i,idx1,idx2)
{
  ...
  {
    #fm=glm(y~x+age+sex+site+bmi+cig_pack_yrs+pc1+pc2+pc3,data=covariates,family=binomial)
    #fm=glm(y~x+age+sex+site+pc1+pc2+pc3,data=covariates,family=binomial)
...
save(assoc_min,assoc_1se,file="../result/bca_assoc.RData")
idx=which(res_1se$r2>0.05)
idx=which(res_1se$r2>0.2)
idx1=match(rownames(res_1se)[idx],rownames(assoc_1se))
qqplot(assoc_1se$BE_p[idx1])
qqplot(assoc_1se$EA_p[idx1])
qqplot(assoc_1se$BEA_p[idx1])
check_genes=function(gene="RBBP5")
{
  dong23snp=read.table("../data/Dong23snp.txt",header=T,sep="\t",stringsAsFactors = F)
  generes=data.frame(gene=gene,chr=NA,start=NA,end=NA)
  idx=match(gene,rownames(phenotypepos))
  generes$chr=phenotypepos$chr[idx]
  generes$start=phenotypepos$s1[idx]
  generes$end=phenotypepos$s2[idx]
  bcasnpgenotype=bcagenotype
  snpgeneexpsamplenames=colnames(bcasnpgenotype)
  snpgeneexpsamplenames=strsplit(snpgeneexpsamplenames,"_")
  snpgeneexpsamplenames=sapply(1:length(snpgeneexpsamplenames),function(x){
    tmp=snpgeneexpsamplenames[[x]]
    paste0(tmp[2:length(tmp)],collapse = "_")
  })
  colnames(bcasnpgenotype)=snpgeneexpsamplenames
  
  for (i in 1:length(gene))
  {
    idx=which(rownames(res_1se)==gene)
    selectedsnps=unlist(strsplit(res_1se$selectedsnps[idx],"|",fixed=T))
    idx=match(selectedsnps,rownames(bcasnpgenotype))
    idx1=which(sampletable1$phenoEA_bc==2) #case
    length(idx1) #2514
    idx2=which(sampletable1$phenoBE_bc==2)
    length(idx2) #2176
    covariates=data.frame(age=sampletable1$age[c(idx1,idx2)],sex=sampletable1$sex[c(idx1,idx2)],site=sampletable1$site[c(idx1,idx2)],
                          bmi=sampletable1$bmi_recent_healthy[c(idx1,idx2)],cig_smk=sampletable1$cig_smk_ever[c(idx1,idx2)],cig_pack_yrs=sampletable1$cig_pack_yrs[c(idx1,idx2)],
                          pc1=sampletable1$pc1[c(idx1,idx2)],pc2=sampletable1$pc2[c(idx1,idx2)],pc3=sampletable1$pc3[c(idx1,idx2)],
                          stringsAsFactors = F)
    snpres=data.frame(matrix(NA,nrow=length(idx),ncol=1))
    rownames(snpres)=selectedsnps
    idx3=match(sampletable1$localid[idx1],colnames(bcasnpgenotype))
    idx4=match(sampletable1$localid[idx2],colnames(bcasnpgenotype))
    for (j in 1:length(idx))
    {
      y=c(rep(1,length(idx1)),rep(0,length(idx2)))
      x=as.numeric(bcasnpgenotype[idx[j],c(idx3,idx4)])
      BE_p=NA
      if (sum(is.na(x))<0.6*length(x))
      {
        fm=glm(y~x+age+sex+pc1+pc2+pc3,data=covariates,family=binomial)
...
# association result without adjusting for environmenatal variables, no significant results found
# load("../result/bca_without_ENV_assoc.RData")
# qqplot(assoc_1se$BE_p)
# qqplot(assoc_1se$EA_p)
# qqplot(assoc_1se$BEA_p)

...
quantile(CCAout1$u)
CCAout1$v[which(rownames(CCAZ)==gene)] #[1] 0.3436551
quantile(CCAout1$v)
# 0%        25%        50%        75%       100% 
# 0.00000000 0.00000000 0.06503782 0.23332023 0.36591574 
rownames(CCAZ)[which.max(abs(CCAout1$v))] #"KIAA0892"
x=matrix(CCAout1$u,nrow = 1)
x=x%*%CCAX1
y=matrix(CCAout1$v,nrow=1)
y=y%*%as.matrix(CCAZ)
fit=glm(as.numeric(y)~as.numeric(x))
plot(as.numeric(y)~as.numeric(x))
abline(fit)
cor(as.numeric(y),as.numeric(x))

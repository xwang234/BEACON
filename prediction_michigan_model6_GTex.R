...
load("/fh/fast/stanford_j/Xiaoyu/QTL/data/GTEx/genotype_phenotype_EC_junction.RData") #read_GTEx_EC.R
#geneexp
phenotype=GE
phenotypepos=GE_pos
...
#save(snp,snppos,phenotype,phenotypepos,covariate,file="../result/GTExjunctiondatafor_prediction.RData")
...
compute_cor_arow_fusion=function(i,ncv=10,distcutoff=5e5)
{
  ...
  if (length(idx)>1)
  {
    ...
    cvfit=tryCatch(
      {
        ### I change alpha to 0.5 for better variable selection when highly correlated features
        cv.glmnet(data.matrix(X),Y,nlambda=100,nfolds=10,alpha=0.5,standardize=F)
      },
      error=function(e)
      {
        return(F)
...
#compute
Sys.time()
#a caes fusion failed to buid a model
test=compute_cor_arow(i=31036,ncv=10,distcutoff = 5e5)
# 0%          25%          50%          75%         100% 
# 0.0006110845 0.0218953444 0.0546338664 0.1068212042 0.2770738504
test1=compute_cor_arow_fusion(i=31036,ncv=10,distcutoff = 5e5)
#a case we failed
test2=compute_cor_arow(i=16116,ncv=10,distcutoff = 5e5)
test3=compute_cor_arow_fusion(i=16116,ncv=10,distcutoff = 5e5)
# 0%        25%        50%        75%       100% 
# 0.04532071 0.14197026 0.19553992 0.25768079 0.61472439
#a case has high r2, fusion failed 
test4=compute_cor_arow(i=30658,ncv=10,distcutoff = 5e5)
test5=compute_cor_arow_fusion(i=30658,ncv=10,distcutoff = 5e5)
Sys.time()
# #parallel
#salloc -t 6-1  -n 79 mpirun -n 1 R --interactive
#load("/fh/fast/dai_j/BEACON/BEACON_GRANT/result/GTExjunctiondatafor_prediction.RData")
library(Rmpi)
...
cmd=paste0("./bca_extract_genotype2.sh ",prefix)
system(cmd,wait = T)
...
if (!exists("phenotypepos")) load("/fh/fast/dai_j/BEACON/BEACON_GRANT/result/GTExjunctiondatafor_prediction.RData") #phenotypepos

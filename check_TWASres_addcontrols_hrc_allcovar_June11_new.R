...
junction_new_pc6=check_organ(outfolder=outfolder,outfolder1="/fh/fast/dai_j/BEACON/BEACON_GRANT/result/dist500K_GTEx_junction_HRC_MAF005_rmhighcor",opt="heritability",plotopt=T)
...
stomach_new_pc6=check_organ(outfolder=outfolder,outfolder1="/fh/fast/dai_j/BEACON/BEACON_GRANT/result/dist500K_GTEx_stomach_HRC_MAF005_rmhighcor",opt="heritability",plotopt=T)
...
blood_new_pc6=check_organ(outfolder=outfolder,outfolder1="/fh/fast/dai_j/BEACON/BEACON_GRANT/result/dist500K_GTEx_blood_HRC_MAF005_rmhighcor",opt="heritability",plotopt=T)
...
mucosa_new_pc6=check_organ(outfolder=outfolder,outfolder1="/fh/fast/dai_j/BEACON/BEACON_GRANT/result/dist500K_GTEx_mucosa_HRC_MAF005_rmhighcor",opt="heritability",plotopt=T)
...
muscularis_new_pc6=check_organ(outfolder=outfolder,outfolder1="/fh/fast/dai_j/BEACON/BEACON_GRANT/result/dist500K_GTEx_muscularis_HRC_MAF005_rmhighcor",opt="heritability",plotopt=T)
...
adipose_new_pc6=check_organ(outfolder=outfolder,outfolder1="/fh/fast/dai_j/BEACON/BEACON_GRANT/result/dist500K_GTEx_adipose_HRC_MAF005_rmhighcor",opt="heritability",plotopt=T)
                      ...
                      r2cutoff=0,opt="old")
{
  ...
  for (i in 1:length(tissues))
  {
    ...
    if (opt %in% c("newpc4","newpc6"))
    {
      ...
      outfolder=paste0("/fh/fast/dai_j/BEACON/BEACON_GRANT/result/dist500K_GTEx_",tissue,"_HRC_MAF005_rmhighcor_allcovar")
    }
    load(paste0(outfolder,"/preidiction_michigan_model.RData"))
    load(paste0(outfolder,"/skat_res.RData"))
...
#
write.csv(genelist_heritability,file="../result/SKAT_TWAS_newpc6_genes_allcovar_heritability.csv",row.names = F)
...
write.csv(genelist01_newpc6,file="../result/SKAT_TWAS_newpc6_genes_allcovar_r2_01_.csv",row.names = F)
...
for (i in 1:length(organs))
{
  ...
  #res4=data.frame(tissue=rep(organ,nrow(assoc_min_pc6)),gene=rownames(assoc_min_pc6),type="EAvsBE",p=assoc_min_pc6$BEA_p,fdr=p.adjust(assoc_min_pc6$BEA_p,method="fdr"),fwer=p.adjust(assoc_min_pc6$BEA_p,method="bonferroni"))
  
  res=rbind(res1,res2,res3)
  allres0=rbind(allres0,res)
}

allres0$FWER=NA
allres0$FDR=NA
allres0$FDR=p.adjust(allres0$p,method="fdr")
allres0$FWER=p.adjust(allres0$p,method="bonferroni")

qqplot(allres0$p)
hist(allres0$p,main="",xlab="p-value",cex.axis=1.3,cex.lab=1.3,col="blue",probability = T)
organs=c("adipose","blood","junction","mucosa","muscularis","stomach")
allres=NULL
for (i in 1:length(organs))
{
...
qqplot(allres$p)
hist(allres$p,main="",xlab="p-value",cex.axis=1.3,cex.lab=1.3,col="blue",probability = T)
plot(allres$fdr,allres$FDR,cex.axis=1.3,cex.lab=1.3,xlab="FDR in each organ & comparision",ylab="FDR in combined data")
abline(0,1,col="red")
plot(-log10(allres$fdr),-log10(allres$FDR),cex.axis=1.3,cex.lab=1.3,xlab="FDR in each organ & comparision (-log10)",ylab="FDR in combined data (-log10)")
abline(0,1,col="red")
par(mar=c(6,4,2,1))
hist(allres$p,col="blue",cex.lab=1.3,cex.axis=1.3,xlab="p-value",main="")
genelist_fwer=read.csv("../result/NEWGenesValidate_allcovar_r2_01_FWER_1000G.csv")
genelist_fwer$allFWER=genelist_fwer$allFDR=genelist_fwer$pvalue=NA
for (i in 1:nrow(genelist_fwer))
{
  tmp=unlist(strsplit(genelist_fwer$type[i],";"))
  idx=which(allres$tissue==genelist_fwer$tissue[i] & allres$type %in% tmp & allres$gene==genelist_fwer$gene[i])
  idx1=match(tmp,allres$type[idx])
  genelist_fwer$pvalue[i]=paste0(scinumber(allres$p[idx[idx1]]),collapse = ";")
  genelist_fwer$allFDR[i]=paste0(scinumber(allres$FDR[idx[idx1]]),collapse = ";")
  genelist_fwer$allFWER[i]=paste0(scinumber(allres$FWER[idx[idx1]]),collapse = ";")
}
write.csv(genelist_fwer,file="../result/NEWGenesValidate_allcovar_r2_01_FWER_1000G_newFDR.csv",row.names = F)

genelist_fdr=read.csv("../result/NEWGenesValidate_allcovar_r2_01_FDR_1000G.csv")
genelist_fdr$allFWER=genelist_fdr$allFDR=genelist_fdr$pvalue=NA
for (i in 1:nrow(genelist_fdr))
{
  tmp=unlist(strsplit(genelist_fdr$type[i],";"))
  idx=which(allres$tissue==genelist_fdr$tissue[i] & allres$type %in% tmp & allres$gene==genelist_fdr$gene[i])
  idx1=match(tmp,allres$type[idx])
  genelist_fdr$pvalue[i]=paste0(scinumber(allres$p[idx[idx1]]),collapse = ";")
  genelist_fdr$allFDR[i]=paste0(scinumber(allres$FDR[idx[idx1]]),collapse = ";")
  genelist_fdr$allFWER[i]=paste0(scinumber(allres$FWER[idx[idx1]]),collapse = ";")
}
write.csv(genelist_fdr,file="../result/NEWGenesValidate_allcovar_r2_01_FDR_1000G_newFDR.csv",row.names = F)
#new results?
idx1=which(allres$FWER<0.05)
idx2=which(allres$FDR<0.05 & allres$FWER>=0.05)
foundres=rbind(genelist_fwer,genelist_fdr)
FDRres=allres[c(idx1,idx2),]
# FDRres$cutoff="FDR"
# FDRres$cutoff[1:length(idx1)]="FWER"
# FDRres$newgene=FDRres$new=F
# for (i in 1:nrow(FDRres))
# {
#   idx=which(foundres$gene==FDRres$gene[i] & foundres$tissue==FDRres$tissue[i])
#   tmp=NULL
#   for (j in 1:length(idx))
#   {
#     tmp1=unlist(strsplit(foundres$type[idx[j]],";"))
#     tmp=c(tmp,tmp1)
#   }
#   if (!FDRres$type[i] %in% tmp)
#   {
#     FDRres$new[i]=T
#   }
#   idx=which(foundres$gene==FDRres$gene[i])
#   if (length(idx)==0) FDRres$new[i]=T
# }

get_genelist_fromtable=function(mytable=FDRres)
{
  genetable=NULL
  for (i in 1:nrow(mytable))
  {
    tissue=mytable$tissue[i]
    outfolder=paste0("/fh/fast/dai_j/BEACON/BEACON_GRANT/result/dist500K_GTEx_",tissue,"_HRC_MAF005_rmhighcor_allcovar")
    load(paste0(outfolder,"/preidiction_michigan_model.RData"))
    gene=mytable$gene[i]
    idx1=which(rownames(res_min)==gene)
    idx2=which(gtexv8_ge_anno$Symbol==gene & gtexv8_ge_anno$V3=="gene")[1]
    generes=data.frame(R2=res_min$r2[idx1],numsnp=res_min$numselectedsnp[idx1],
                       chr=gtexv8_ge_anno$Chromosome[idx2],position=gtexv8_ge_anno$start[idx2],stringsAsFactors = F)
    genetable=rbind(genetable,generes)
  }
  gr_genetable=GRanges(seqnames = genetable$chr,ranges=IRanges(start=genetable$position,width=1))
  ...
  genetable=cbind.data.frame(mytable,genetable)
  idx=order(genetable$type,genetable$tissue,genetable$dist_gwassnp)
  genetable=genetable[idx,]
  idx=which(is.na(genetable$dist_gwassnp) | genetable$dist_gwassnp>10000000)
  genetable$new=F
...
genelist_allFDR=get_genelist_fromtable()
write.csv(genelist_allFDR,file="../result/SKAT_TWAS_newpc6_genes_allcovar_AllFDR05.csv",row.names = F)
...
gwasdat_beacon_hrc=read_gwassnp(genotypefolder="/fh/fast/dai_j/BEACON/BEACON_GRANT/data/imputation/beacondbgapcontrol_qc_hrc/")
tmp1=gwasdat_beacon$gwasdat
tmp2=gwasdat_beacon_hrc$gwasdat
tmp3=intersect(colnames(tmp1),colnames(tmp2))
tmp1=tmp1[,match(tmp3,colnames(tmp1))]
tmp2=tmp2[,match(tmp3,colnames(tmp2))]
for (i in 1:ncol(tmp1))
{
  tmp1[,i]=as.numeric(tmp1[,i])
  tmp2[,i]=as.numeric(tmp2[,i])
}
sum(tmp1!=tmp2)/nrow(tmp1)/ncol(tmp1) #[1] 0.007985134
...
#cp ../result/dong26snp_addcontrols_genotypedat.txt ../result/dong26snp_addcontrols_genotypedat_24snp.txt 
#add two missing snps "rs9918259"  "rs75783973"
gwasdat_beacon1=read_gwassnp(genotypefolder ="/fh/fast/dai_j/BEACON/BEACON_GRANT/data/imputation/beacondbgapcontrol_qc_1000gphase1_maf001_snp/")
gwasdat_cambridge1=read_gwassnp(genotypefolder ="/fh/fast/dai_j/BEACON/BEACON_GRANT/data/imputation/cambridgewtccc_qc_1000gphase1_maf001_snp/")
all(colnames(gwasdat_beacon1$gwasdat)==colnames(gwasdat_cambridge1$gwasdat)) #T
allgwasdat1=rbind(gwasdat_beacon1$gwasdat,gwasdat_cambridge1$gwasdat)
all(colnames(bcagenotype) %in% rownames(allgwasdat1)) #T
idx=match(colnames(bcagenotype),rownames(allgwasdat1))
allgwasdat1=allgwasdat1[idx,]
allgwasdat_all=cbind(allgwasdat,allgwasdat1)
write.table(allgwasdat_all,file="../result/dong26snp_addcontrols_genotypedat.txt",sep="\t",quote=F)

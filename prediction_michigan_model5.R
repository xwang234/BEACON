#!/usr/bin/env Rscript
#Only use TCGA genotyped SNPs
# #get all the data
library(data.table)
load("/fh/fast/dai_j/CancerGenomics/EAprogression/data/TCGA_EAC_Genotype_Genexp.RData")
# snp=genotypedata
# snp6anno=read.table(file="/fh/fast/dai_j/CancerGenomics/Tools/database/other/GenomeWideSNP_6.na35.annot.csv",skip=18,header=T,sep=",",stringsAsFactors=F)
# all(rownames(genotypedata) %in% snp6anno$Probe.Set.ID) #TRUE
# idx=match(rownames(genotypedata),snp6anno$Probe.Set.ID)
# snppos=data.frame(chr=snp6anno$Chromosome[idx],pos=as.integer(snp6anno$Physical.Position[idx]),A=snp6anno$Allele.A[idx],B=snp6anno$Allele.B[idx],rsid=snp6anno$dbSNP.RS.ID[idx],gene=snp6anno$Associated.Gene[idx],stringsAsFactors = F)
# snppos$chr=gsub(23,"X",snppos$chr)
# idx=is.na(snppos$pos)
# snp=snp[!idx,]
# snppos=snppos[!idx,]
# rownames(snppos)=rownames(snp)
#genotype data use imputed ones on SNP6, to make sure the genotype data are consistent with BCA
snp=NULL
snppos=NULL
for (i in 1:22)
{
  tmp=fread(paste0("/fh/fast/dai_j/CancerGenomics/EAprogression/result/imputation_vcf/TCGAtumors_chr",i,"_flip1.raw"),header = T)
  ...
  tmp1=fread(paste0("/fh/fast/dai_j/CancerGenomics/EAprogression/result/imputation_vcf/TCGAtumors_chr",i,"_flip1.bim"))
  tmp1=as.data.frame(tmp1)
  rownames(tmp)=paste0(i,":",tmp1$V4,"_",tmp1$V5)
  tmp2=data.frame(chr=tmp1$V1,pos=as.integer(tmp1$V4),minor=tmp1$V5,major=tmp1$V6,stringsAsFactors = F)
  rownames(tmp2)=rownames(tmp)
  snp=rbind(snp,tmp)
  snppos=rbind(snppos,tmp2)
}
tmp=unlist(strsplit(rownames(snppos),"_"))
rownames(snppos)=tmp[seq(1,length(tmp),2)] #make sure snppos has the correct format of rownames, used for predict_geneexp
#geneexp
...
#save(snp,snppos,phenotype,phenotypepos,copynumber,mutation,covariate,file="../result/TCGAdatafor_prediction_SNP6.RData")
...
#load data
load("/fh/fast/dai_j/BEACON/BEACON_GRANT/result/TCGAdatafor_prediction_SNP6.RData")
library(glmnet)
...
mpi.remote.exec(load("/fh/fast/dai_j/BEACON/BEACON_GRANT/result/TCGAdatafor_prediction_SNP6.RData"))
...
res_min[4777,]
# r2 glmflag numselectedsnp numtotalsnp
# DDX49 0.005077759       1              2         110
# selectedsnps                   selectedsnps_coeff
# DDX49 19:18782377_A|19:18749354_A 0.138762303588362|-0.246425243353333
# p_cn p_mutation   p_stage     p_age  p_gender
# DDX49 5.211601e-12         NA 0.6561981 0.7915269 0.6526358
idx=res_min$glmflag==1
sum(idx)
1sum(idx)/nrow(phenotypepos)
quantile(res_min$r2[idx])
quantile(res_min$numselectedsnp[idx])
...
predict_geneexp=function(i=1,modeltable=res_min)
{
  ...
  if (length(intersect(selectedsnps,rownames(bcagenotype_chunk)))<length(selectedsnps))
  {
    ...
    for (j in 1:length(missingsnps))
    {
      ...
      if (length(idx1)>0)
      {
        idx=which(bcabim[,2]==tmp[1] & bcabim[,6]==tmp[2]& bcabim[,5]==snppos[idx1,4])
        if (length(idx)>0)
        {
          idx=idx[1]
          idx1=which(selectedsnps==missingsnps[j])
          selectedsnps[idx1]=paste0(bcabim[idx,2],"_",bcabim[idx,5])
          correctedsnps=c(correctedsnps,paste0(bcabim[idx,2],"_",bcabim[idx,5]))
...
##salloc -t 1-1 --mem-per-cpu 32G -n 21 --partition=largenode mpirun -n 1 /app/easybuild/software/R/3.3.3-foss-2016b-fh1/bin/R --interactive
if (!exists("phenotypepos")) load("/fh/fast/dai_j/BEACON/BEACON_GRANT/result/TCGAdatafor_prediction_SNP6.RData") #phenotypepos
...
genes=rownames(res_min)[res_min$glmflag==1 & res_min$r2>0.05]
idx=match(genes,rownames(assoc_min))
qqplot(assoc_min[idx,1])
min(p.adjust(assoc_min[idx,7]),na.rm=T)
assoc_min[2507,]
# BE_p      BE_fdr         EA_p     EA_fdr     BEA_p BEA_fdr
# DDX49 3.742093e-07 0.003886164 8.169532e-06 0.08483242 0.5618361       1
# BEEA_p     BEEA_fdr
# DDX49 4.694844e-08 0.0004875595

...
extract_vcf1(){
  ...
  xargs -a $tabixreg -I {} tabix -f /fh/fast/dai_j/BEACON/BEACON_GRANT/data/imputation/bca_1000g/chr${chr}_filter_hg19tohg38_flip.vcf.gz {} >> $outfolder/chr${chr}_1.select.dose.vcf
...
do
  ...
  then
    tabix -H /fh/fast/dai_j/BEACON/BEACON_GRANT/data/imputation/bca_1000g/chr${chr}_filter_hg19tohg38_flip.vcf.gz >$outfolder/chr${chr}.head.select.dose.vcf
    extract_vcf1 $chr &
    extract_vcf2 $chr &
  fi
done

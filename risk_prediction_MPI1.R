...
#request large node 
#salloc -t 2-1 --mem-per-cpu 32G -n 21 --partition=largenode mpirun -n 1 Rscript 
...
compute_pvalue=function(i=1,pheno,genodat,covdat)
{
  ...
  fit1 <- glm(I(pheno==2)~genotype+sex+age+ev1_bca+ev2_bca+ev3_bca+ev4_bca,family=binomial,data=covdat)
...
mpi_compute_pvalue <- function(nsnps=1000){
  ...
  for (ii in 1:nchunks)
  {
    ...
    Covariate1 <- Covariate[sampletable$site<30,]
    
    pheno=sampletable$phenoBE_bca[sampletable$site<30]
    genodat=allgenotype[seqs,sampletable$site<30]
    idx=!is.na(pheno)
    pheno=pheno[idx]
    genodat=genodat[,idx]
    ...
    pheno=sampletable$phenoEA_bca[sampletable$site<30]
    genodat=allgenotype[seqs,sampletable$site<30]
    idx=!is.na(pheno)
    pheno=pheno[idx]
    genodat=genodat[,idx]
    ...
    pheno=sampletable$phenoEABE_bca[sampletable$site<30]
    genodat=allgenotype[seqs,sampletable$site<30]
    idx=!is.na(pheno)
    pheno=pheno[idx]
    genodat=genodat[,idx]
    ...
    pheno[pheno==1]=2
    pheno <- pheno[sampletable$site<30]
    genodat=allgenotype[seqs,sampletable$site<30]
    idx=!is.na(pheno)
    pheno=pheno[idx]
    genodat=genodat[,idx]
